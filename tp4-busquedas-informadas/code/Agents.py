...
class GoalAgentAStar(GoalBasedAgent2D):
    def __init__(self, env: Environment):
        super(GoalAgentAStar, self).__init__(env)

    def think(self) -> list:
        # Init queue and discovered dict (hashed)
        frontier = PriorityQueue()
        frontier.push(self._heuristic(self.env.init_pos), self.env.init_pos)
        discovered = {self.env.init_pos: None}  # And save parents
        ...
        while len(frontier) > 0:
            ...
            # Remove the heuristic from the weight
            actual_weight -= self._heuristic(actual)
            ...
            if actual == self.env.goal_pos:
                ...
                borders = self._get_borders(actual, discovered)
                for border in borders:
                    # Push into the queue the weight and the heuristic.
                    frontier.push(actual_weight+1 + self._heuristic(border), border)
                    discovered[border] = actual

        # Return failure -> empty path
        return []

    def _heuristic(self, pos: tuple):
        # L2 distance. Size always is 24bytes. No need to round.
        dx = self.env.goal_pos[0] - pos[0]
        dy = self.env.goal_pos[1] - pos[1]
        # l1 = (dx**2 + dy**2)    # Works really well, but not optimal.
        l2 = (dx**2 + dy**2)**.5
        return l2
...
if __name__ == '__main__':
    ...
    for agent_type in [GoalAgentBFS, GoalAgentDFS, GoalAgentDLS, GoalAgentUCS, GoalAgentAStar]:
